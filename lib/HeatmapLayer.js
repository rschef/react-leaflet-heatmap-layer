...
var HeatmapLayer = function (_MapLayer) {
  ...
  HeatmapLayer.prototype.componentDidMount = function componentDidMount() {
    ...
    var canAnimate = leafletMap.options.zoomAnimation && _leaflet2.default.Browser.any3d;
  ...
  HeatmapLayer.prototype._animateZoom = function _animateZoom(e) {
    var leafletMap = this.props.leaflet.map;
    var scale = leafletMap.getZoomScale(e.zoom);
    var offset = leafletMap._getCenterOffset(e.center)._multiplyBy(-scale).subtract(leafletMap._getMapPanePos());
  ...
  HeatmapLayer.prototype.reset = function reset() {
    var leafletMap = this.props.leaflet.map;
    var topLeft = leafletMap.containerPointToLayerPoint([0, 0]);
    ...
    if (this._heatmap && !this._frame && !leafletMap._animating) {
  ...
  HeatmapLayer.prototype.redraw = function redraw() {
    ...
    var maxZoom = this.props.maxZoom === undefined ? leafletMap.getMaxZoom() : this.getMaxZoom(this.props);

    var v = 1 / Math.pow(2, Math.max(0, Math.min(maxZoom - leafletMap.getZoom(), 12)));

    var cellSize = r / 2;
    var panePos = leafletMap._getMapPanePos();
...
exports.default = (0, _reactLeaflet.withLeaflet)(HeatmapLayer);
